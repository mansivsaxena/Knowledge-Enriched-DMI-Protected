...
def apply_defense(logits, defense="none", noise_sigma=0.01, smooth_alpha=0.1):
    """
    Phase3 Related Change:
    Apply output-level defense to logits.
    defense: "none" | "noise" | "smooth"
    """
    if defense == "noise":
        # Add Gaussian noise to the logits
        noise = torch.randn_like(logits) * noise_sigma
        return logits + noise

    elif defense == "smooth":
        # Apply label smoothing to the logits
        probs = F.softmax(logits, dim=1)
        K = probs.shape[1]
        probs = (1 - smooth_alpha) * probs + smooth_alpha / K
...
def dist_inversion(G, D, T, E, iden, itr, lr=2e-2, momentum=0.9, lamda=100, iter_times=1500, clip_range=1, improved=False, num_seeds=5, 
                   # Phase3: new args for defense 
                   defense="none",
                   noise_sigma=0.01,
                   smooth_alpha=0.1):
    # Phase1: saving time for each attack
    ...
    for i in range(iter_times):
        ...
        out = T(fake)[-1]
        # Phase3: apply defense if provided
        if defense and defense != "none":
            out = apply_defense(out, defense, noise_sigma, smooth_alpha)
...
def inversion(G, D, T, E, iden, itr, lr=2e-2, momentum=0.9, lamda=100, iter_times=1500, clip_range=1, improved=False, num_seeds=5,
    ...
    acc_var = statistics.variance(res)
    acc_var5 = statistics.variance(res5)
    seed_var = torch.var(seed_acc, dim=1)
    print("Acc:{:.2f}\tAcc_5:{:.2f}\tAcc_var:{:.4f}\tAcc_var5:{:.4f}".format(acc, acc_5, acc_var, acc_var5))
    print("seeds variance:", seed_var)
	
    # Phase1: saving time for each attack
    total_time = time.time() - overall_start
    return acc, acc_5, acc_var, acc_var5, total_time #Phase1: returning total_time for each attack